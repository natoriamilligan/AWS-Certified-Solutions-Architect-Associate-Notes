 ...
 - Use Cases
  ...
  ## Security
   ...
   - NOTE: an IAM principle can access an S3 object if:
     ...
     2. AND theres no explicit DENY. the DENY of any IAM policy will override any S3 bucket policy
 ...
 ## Versioning
  ...
  ## Replication (CRR & SRR)
   ...
   - to replicate existing objects use S3 Batch Replication
     ...
     - best to do this after you create the bucket, not at time of creating the bucket
...
## Storage Classes
 ...
 ### Express One Zone
  - high performance
  - single AZ (you get to choose)
  - objects stored in a Directory Bucket (bucket in a single AZ)
  - handles 100,000s requests per second with signle digit milisecond latency
  - up to 10x better performance than S3 Standard (50% lower costs)
  - high durability and availability (99.5)
  - co locate your storage and compute resources in the same AZ
  - use cases : latency sensitive apps, data-intensive apps, AI and ML training, financial modeling
  - best integrated with SageMaker Modeling Training, Athena, EMR Glue

## Moving Between Storage Classes
 - you can transition objects btw sorage classes
 - for infrequently  accessed objects, move them to standard IA
 - for archive objects that you dont need fast, move them to glacier or glacier deep dive
 - moving objects can be automated using Lifecycle Rules
   ### Lifecycle Rules
    - transition actions : configure objects to transition to another storage class
    - expiration actons : configure objects to expire (delete) after some time
   ...
   ### Storage Class Analysis
    ...
    - recommendatons for standard and standard IA
